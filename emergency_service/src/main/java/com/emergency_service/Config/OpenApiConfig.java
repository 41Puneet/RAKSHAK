package com.emergency_service.Config;
import org.springframework.context.annotation.Bean; import org.springframework.context.annotation.Configuration; import io.swagger.v3.oas.models.OpenAPI; import io.swagger.v3.oas.models.info.Info;
@Configuration public class OpenApiConfig { @Bean public OpenAPI emergencyApi() { return new OpenAPI().info(new Info().title("RAKSHAK Emergency API").version("1.0.0").description("Emergency creation, cancellation and control-room queries")); } }
